        ...
        <div class="col-md-5 col-lg-6 col-xl-4 px-lg-5 d-flex align-items-center">
          <div class="w-100 py-5">
            <div class="text-center"><img src="<?= base_url('assets/bootstrapdashboard/IMG/svg/brand-1.svg')?>" alt="..." style="max-width: 6rem;" class="img-fluid mb-4">
              <h1 class="display-4 mb-3"><?= lang('ui_register')?></h1>
            </div>
            <!-- <form  method = "post" action = "<?php echo base_url('login/dologin');?>" class="text-left form-validate"> -->
            
              <div class="form-group">
                <label>Player</label>
                <input id = "playername" name="playername" type="text" autocomplete="off" required="" class="form-control">
              </div>
              
              <!-- Submit-->
              <button onclick = "register();" class="btn btn-lg btn-block btn-primary mb-3"><?= lang('ui_register')?></button>
              <!-- Link-->
...
<script>
  ...
  function register(){
    ...
    $.get("<?= base_url('api/player/register?isbrowser=true&playername=')?>" +player, function(player, status){
      if(player['data'] != null){
        if(player['data']['Name']){
          
        }
      } else { 
        if(player['status']['code'] == 2){
          alert("Player sudah didaftarkan")
        }
      }
    });
  }
